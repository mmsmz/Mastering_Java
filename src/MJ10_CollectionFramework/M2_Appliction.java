...
//        int itm1 =  (int)word.get(2);
//        int itm2 =  (int) word.get(3);
//        System.out.println(itm1 +itm2); // ouput : 19
        ...
        for (int number : numbers){
            System.out.println(number); // 200,45,1000
        }
    ...
    // refer how practical data structures and algorithms
        ...
        // LinkedList
 /* The LinkedList has a sort of  like, imagine you have a train with a different box carts connected together with links and that's exactly how to envision this LinkedList data structure. When we need to add more data all it does is attaches a new cart at the end of the train and continues to do that and it's pretty dynamic. */

  /*So the LinkedList data structure is actually better for manipulating the data compared to the ArrayList you see when you want to remove something from the beginning of the ArrayList all of the items that follow any to be copied back to the zeroth index position back one slot over a LinkedList uses something called a doubly LinkedList data structure which you can review.*/

// ** But basically the LinkedList is faster for manipulating and LinkedList is faster for retrieving data to retrieve values from a LinkedList.
// ** It would have to be traversed completely to find the particular element whereas an ArrayList much faster retrieval you just give the index position and there is no traversal. It just immediately goes to the particular slot and gives us that data.
